...
    }
    ...
    // ⭐ 4. NOVO: PROVERA STATUSA API-ja (Kada nema 'action' parametra)
    // Ovo rešava problem Unknown action na root URL-u /api
    if (!action || action === "") {
        // Šaljemo prazan GET ka GAS-u. GAS će vratiti { ok: true, message: 'Booking API radi.' }
        const upstream = await fetchGAS(new URLSearchParams()); 
        if (!upstream.ok) return res.status(502).json(upstream);
        return res.status(200).json(upstream.data);
    }
    
    // Vraća se greška 404 samo ako je 'action' definisan ali nije prepoznat
    // ...
    return res.status(404).json({ ok: false, error: "OVO JE NOVA VERZIJA KODA, ALI AKCIJA NIJE PREPOZNATA" });
  }
